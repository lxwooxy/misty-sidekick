...
def head_touched(data):
    ...
    message = data["message"] # Gets the message from the event data

    # Check if the head is touched
    if message["isContacted"] and message["sensorId"] == "cap":
        
        # Gets the position of the touch sensor 
        # It can be one (or more) of ["HeadFront", "HeadBack", "HeadRight", "HeadLeft", "Chin", "Scruff"]
        ...
        if sensor_position in ["HeadFront", "HeadBack"]:
            ...
            misty.display_image("e_Love.jpg")
            misty.move_arms(-90, -90) # Arms down
