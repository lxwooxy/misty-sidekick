...
def curr_position(data):
    ...
    if data["message"]["sensorId"] == "ahp":
        ...
        print("Current Pitch:", curr_head_pitch)
    if data["message"]["sensorId"] == "ahy":
        curr_head_yaw = data["message"]["value"]
        print("Current Yaw:", curr_head_yaw)

# Function to get the current head position
def get_pos():
...
def person_detection(data):
    ...
    if data["message"]["confidence"] >= 0.6:
        ...
        # Human tracking y-error calculation
...
# Start Misty's object detection
