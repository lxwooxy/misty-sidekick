...
def bumper_touched(data):
    """Misty performs 'Miku Miku Beam' when the bumper is touched."""
    message = data["message"]
...
LINEAR_SPEED = 30
ANGULAR_SPEED = 30
DRIVE_INTERVAL = 0.2  # seconds to hold movement before stopping

def drive(linear, angular):
    misty.drive(linear, angular)

def stop_drive():
    misty.halt()

# Head movement offsets
head_pitch = 0
head_roll = 0
head_yaw = 0

def on_press(key):
    global head_pitch, head_roll, head_yaw
    try:
        if key.char == 'w':
            drive(LINEAR_SPEED, 0)
        elif key.char == 's':
            drive(-LINEAR_SPEED, 0)
        elif key.char == 'a':
        ...
        if key == keyboard.Key.up:
            head_pitch = max(head_pitch - 20, -40)  # Look up
        elif key == keyboard.Key.down:
            head_pitch = min(head_pitch + 20, 20)   # Look down
        elif key == keyboard.Key.left:
            head_yaw = max(head_yaw - 20, -40)      # Look left
        elif key == keyboard.Key.right:
            head_yaw = min(head_yaw + 20, 40)       # Look right
        elif key == keyboard.Key.space:
            print("✨ Jump!")
            misty.play_audio("s_Excitement.wav")
            misty.display_image("e_Joy2.jpg")
            misty.move_arms(-90, -90)
            time.sleep(0.5)
            misty.move_arms(90, 90)
...
def on_release(key):
    ...
    if key == keyboard.Key.esc:
        print("👋 ESC pressed. Exiting drive loop.")
        return False


def start_keyboard_control():
    print("🕹️ WASD control ready. Hold keys to drive, press ESC to stop.")
